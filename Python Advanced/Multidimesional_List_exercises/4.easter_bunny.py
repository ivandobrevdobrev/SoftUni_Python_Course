...
for direction, positions in directions.items():
    ...
    while 0 <= row < size and 0 <= col < size:
        ...
        row += positions[0]  # придвижваме заека на валидната позиция, на която е взел яйце
        col += positions[1]
    if collected_eggs >= max_collected_egg:
        max_collected_egg = collected_eggs
